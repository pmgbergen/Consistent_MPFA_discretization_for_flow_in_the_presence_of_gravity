...
def solve_two_phase_flow_gcmpfa(g):
    ...
    _, _, _, _, div_g  = pp.Mpfa("flow")._local_discr(
        g, perm, bound_cond, vector_source=True, inverter="python"
    )
    ...
    while t < T:
        ...
        # recalculate transmissibilities with mobility
        lk_xx = np.zeros(g.num_cells)
        lk_xy = np.zeros(g.num_cells)
        lk_yy = np.zeros(g.num_cells)
        lk_xx[:] = (k1(s[:]) + k2(s[:])) * perm_xx_f(xc[0], xc[1])
        lk_yy[:] = (k1(s[:]) + k2(s[:])) * perm_yy_f(xc[0], xc[1])
        lk_xy[:] = (k1(s[:]) + k2(s[:])) * perm_xy_f(xc[0], xc[1])

        lk = pp.SecondOrderTensor(2, lk_xx, kyy=lk_yy, kxy=lk_xy)

        # discretize gravity term
        kr1g1 = np.zeros((2, g.num_cells))
        kr1g1[0,:] = k1(s[:]) * rho1 * gx
        kr1g1[1,:] = k1(s[:]) * rho1 * gy
        kr1g1 = kr1g1.ravel('F')

        kr2g2 = np.zeros((2, g.num_cells))
        kr2g2[0,:] = k2(s[:]) * rho2 * gx
        kr2g2[1,:] = k2(s[:]) * rho2 * gy
        kr2g2 = kr2g2.ravel('F')

        flux, bound_flux, _, _  = pp.Mpfa("flow").mpfa(
            g, lk, bound_cond, inverter="python"
        )
        fluxtot_g = div_g * (kr1g1 + kr2g2)
        ...
        fluxtot = flux * p + bound_flux * p_bound + fluxtot_g
        ...
        #kr1, kr2, f2, kr1_g, kr2_g, f2_g = get_hybrid_upwind(g, s, fluxtot)
        #grad_p = flux_k * p + bound_flux_k * p_bound
        #q2 = kr2 * (grad_p + flux_g2) + (kr1 * f2 - kr1_g * f2_g) * (flux_g2 - flux_g1)
        #q1 = kr1 * (grad_p + flux_g1) + (kr1 * f2 - kr1_g * f2_g) * (flux_g1 - flux_g2)
        q2[bnd_faces]=0
        q1[bnd_faces]=0
        ...
        if k % 100 == 0:
            ...
            if F > 1. and time_step < max_time_step:
...
def get_hybrid_upwind(g, s, q):
    ...
    return kr1mat, kr2mat, f2mat, kr1_gmat, kr2_gmat, f2_gmat
...
t, s, p, q = solve_two_phase_flow_gcmpfa(g)

# Create a workbook and add a worksheet.
workbook = xlsxwriter.Workbook('gcmpfa_hperm_n64.xlsx')
